...
// Aliased: this function already has a local `netPerSale` (a number, further
// down), and a same-named import would be shadowed by it for the whole body -
// silently, until the line above the local runs and hits its dead zone.
import { netPerSale as storeNet, betterAtLowRate, DELIVERY_FEE_PER_MB } from "../economics.js";
...
export function buildKdpPack({ book, epubName }) {
  ...
  // Delivery is charged per megabyte on every sale, forever. It is invisible
  // until the first royalty report, by which time the book is listed.
  const fileMb = (book.epubBytes || 0) / (1024 * 1024);
  const price = listing.priceUsd || 0;
  const amazon = storeNet("amazon", Math.min(price || 9.99, 9.99), { fileMb });

  if (amazon.delivery >= 0.2) {
    warnings.push(
      `This file is ${fileMb.toFixed(1)} MB. At about $${DELIVERY_FEE_PER_MB.toFixed(2)}/MB, ` +
        `KDP deducts $${amazon.delivery.toFixed(2)} in delivery costs from EVERY sale on the 70% option ` +
        `(net $${amazon.net.toFixed(2)} instead of $${amazon.gross.toFixed(2)}).`,
    );
  }
  if (betterAtLowRate({ price: price || 9.99, fileMb })) {
    warnings.push(
      `At this file size the 35% royalty option pays MORE than 70%, because 35% carries no ` +
        `delivery fee. Compare both on the KDP pricing page before you choose.`,
    );
  }
...
When KDP asks which parts: **text** (AI-generated, then edited)${
    book.imagesGenerated
      ? " **and images** (AI-generated artwork, including the cover)"
      : book.figureCount
        ? " and **images** (generated charts, not AI-generated art)"
        : ""
  }.
> ${book.aiDisclosure || AI_DISCLOSURE}
${
  book.imagesGenerated
    ? `
> **Answer the images question honestly.** KDP asks about text and images
> separately, and this book's pictures were generated by an image model. A
...
The cover to upload is \`cover.svg\` — ${
    book.imagesGenerated
      ? "generated artwork with the title typeset over it"
      : "typographic, with no artwork"
  }. KDP requires a raster image — JPEG or TIFF, ideally 1600 × 2560 px. Convert
before uploading:
...
${
  book.imagesGenerated
    ? `
\`cover-art.png\` is the raw artwork **without** the title. Upload \`cover.svg\`,
not that one: a cover with no title on it will not sell, and KDP will not add
the title for you.

The title is vector text drawn over the picture, not generated by the image
model — which is the only way to be certain it is spelled correctly. Check it
once at thumbnail size before you upload.
`
    : ""
}
